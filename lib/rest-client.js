...
module.exports = class RestClient {
  ...
  constructor(key, secret, livenet= true) {
  ...
  async getKlineData(params) {
    assert(params, 'No params passed');
    assert(params.symbol, 'Parameter symbol is required');
    assert(params.interval, 'Parameter interval is required (1 3 5 15 30 60 120 240 360 720 "D" "M" "W" "Y")');
    assert(params.from, 'Parameter from is required (timestamp in seconds -> e.g. 1581231260)');
